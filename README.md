...
A browser app for private, one-way screen sharing with shared audio. The host starts a stream and sends attendees an invitation link and a password. Attendees open the link, enter the password, and connect automatically. No accounts, installation, voice chat, or approval lobby.
...
Open `http://localhost:3000` in desktop Chrome or Edge. Choose a password (at least 12 characters), then **Share screen & audio**. In the browser picker, select a tab and enable **Share tab audio**. Screen/window audio depends on browser and operating-system support. Capture is rejected if no audio track is present.

After the connection service is ready, copy the invitation and share the password separately. Test with another browser window. Keep the host tab open and the computer awake. **End stream**, closing the host tab, or the browser's **Stop sharing** ends delivery. A new stream creates a new invitation; old invitations do not follow the new session.
...
- Video and shared audio travel directly from the host's browser to each attendee over encrypted WebRTC connections.
...
- Each viewer consumes another stream's worth of host upload and encoding resources. This app targets small private groups, not scale.
...
The player shows actual video dimensions and stream statistics. The host preview is muted to prevent feedback. Attendees receive the captured audio and can adjust volume or fullscreen. If autoplay is blocked, **Play with sound** provides the required user gesture. No `getUserMedia`, microphone, or camera capture is used.
...
The host browser checks a fresh challenge-response before creating any media sender. Passwords and room keys remain in browser memory, never in URLs, storage, signaling metadata, or a database. PBKDF2-HMAC-SHA256 derives a room-specific key; HMAC authenticates the password proof and all subsequent SDP/ICE messages, including media fingerprints and sequence numbers. Viewers are receive-only, and the host rejects inbound media calls.

Share a strong, unique password separately from the invitation. This is shared-password access, not individual identity: anyone with both can watch or forward them. It is not a PAKE; captured authentication transcripts permit offline password guessing, so use the generator or a long unpredictable phrase. Peers learn each other's connection addresses. End the stream and create a new one to replace access credentials.
